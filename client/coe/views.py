...
class CoeCreateView(SuccessMessageMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse("client.details", kwargs={"client": int(self.kwargs["client"])})
...
class CoeDeleteView(DeleteView):
    ...
    def get_success_url(self):
        coe = Coe.objects.get(id=self.kwargs["coe"])
        return reverse("client.details", kwargs={"client": coe.client.id})
...
def delete(request, document):
    ...
    document.delete()
    return redirect(reverse("coe.details", kwargs={"coe": coe.id}) + "#tab_stage")
